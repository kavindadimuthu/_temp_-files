...
const AsgardeoProvider: FC<PropsWithChildren<AsgardeoProviderProps>> = ({
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      ...
      if (exchangeOrganization?.switchToOrganizationId && exchangeOrganization?.switchFromInstanceId !== undefined) {
        // await handleOrganizationTokenExchange();

        const switchToOrganization : Organization = {
          id: exchangeOrganization.switchToOrganizationId,
          name: "",
          orgHandle: "",
        }

        await switchOrganization(switchToOrganization, false)
        return;
      }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          console.warn(
            `Organization token exchange attempt ${attempt + 1} failed. Retrying in ${delayMs / 1000}s...`,
          );
  ...
  /**
   * Handles organization token exchange for parent-sub org scenarios.
   * Implements retry logic to handle delays in parent session availability.
   */
  const handleOrganizationTokenExchange = async (): Promise<void> => {
    setIsUpdatingSession(true);
    setIsLoadingSync(true);

    try {
      await retryWithBackoff(async () => {
        const tokenResponse = await asgardeo.exchangeToken({
          attachToken: false,
          data: {
            client_id: '{{clientId}}',
        ...
        if (!tokenResponse || !(await asgardeo.isSignedIn())) {
          throw new Error('Token exchange completed but session not established');
        }
      ...
      console.error('Organization token exchange failed after all retries:', error);
      throw new AsgardeoRuntimeError(
        `Organization token exchange failed: ${error instanceof Error ? error.message : String(error)}`,
        'asgardeo-organizationTokenExchange-Error',
        'react',
        'Failed to exchange parent access token for organization token.',
      );
    } finally {
  ...
  const switchOrganization = async (organization: Organization, signInRequired?: boolean): Promise<TokenResponse | Response> => {
    try {
      setIsUpdatingSession(true);
      setIsLoadingSync(true);
      const response: TokenResponse | Response = await asgardeo.switchOrganization(organization, signInRequired);
