...
- [12. Migration Guide (Legacy → New)](#12-migration-guide-legacy--new)
- [13. Dependencies](#13-dependencies)
- [14. Risks & Mitigations](#14-risks--mitigations)
- [15. Appendix: React ↔ Vue Mapping](#15-appendix-react--vue-mapping)

---

## 1. Executive Summary

This plan details the complete rewrite of the `@asgardeo/vue` SDK from its current legacy state (built on `@asgardeo/auth-spa`) to a modern, specification-compliant Core Lib SDK built on `@asgardeo/browser` (Platform SDK). The new SDK will:

- Follow the **four-layer architecture** defined in the IAM SDK specification
- Use `@asgardeo/browser` as its parent layer dependency (replacing the legacy `@asgardeo/auth-spa`)
...
|--------|--------------|--------------|
| **Parent SDK** | `@asgardeo/auth-spa` v3.3.2 (legacy) | `@asgardeo/browser` (workspace) |
| **Build tool** | Rollup | esbuild (monorepo standard) |
| **Plugin** | `asgardeoPlugin` — basic Vue plugin with `provide`/`inject` | Full plugin with nested providers |
| **Composables** | `useAsgardeo()` — wraps basic auth context | `useAsgardeo()` + `useUser()` + `useOrganization()` + `useFlow()` + `useTheme()` + `useI18n()` + `useBranding()` + `useFlowMeta()` |
| **Reactive state** | `reactive()` for basic auth state only | Comprehensive reactive state for auth, user, org, flow, theme, i18n |
| **UI Components** | None | Full component library (Actions, Control, Presentation) |
| **Embedded flows** | Not supported | Full App-Native + V2 embedded flow support |
| **Branding/Theme** | Not supported | Theme provider, branding resolution, Base/Styled variants |
| **i18n** | Not supported | i18n provider with `@asgardeo/i18n` integration |
| **Organizations** | Not supported | Full org management (list, switch, create) |
| **TypeScript** | v5.1.6 | Aligned with monorepo (latest) |

### 2.2 What Can Be Reused

- **Nothing from the current Vue SDK should be reused.** The entire implementation is built on the legacy `@asgardeo/auth-spa` client, which is being replaced. The plugin and composable patterns will be reimplemented from scratch using the Browser SDK.
- The existing legacy files should be moved to a `__legacy__/` subdirectory (consistent with how `packages/react/src/__legacy__/` is structured) to preserve backward compatibility during the transition.

### 2.3 Key Differences: React SDK → Vue SDK
...
### 4.1 Move Legacy Code

Move all existing source files under `packages/vue/src/` into `packages/vue/src/__legacy__/`:

```
packages/vue/src/__legacy__/
  ├── auth-api.ts
  ├── index.ts
  ├── public-api.ts
  ├── types.ts
...
Key changes:
- **Dependency**: `@asgardeo/browser: workspace:*` replaces `@asgardeo/auth-spa`
- **Dependency**: `@asgardeo/i18n: workspace:*` added for i18n
- **Build**: esbuild replaces rollup
- **Removed**: All crypto polyfills (`base64url`, `buffer`, `fast-sha256`, `jose`, `randombytes`) — these are handled by `@asgardeo/browser`
- **Removed**: `@asgardeo/js` dependency

### 4.3 Build Configuration (`esbuild.config.mjs`)
...
| `docs/migration.md` | Migration guide from legacy `@asgardeo/vue` (v0.x) |
...
└── src/
    ...
    │
    ├── __legacy__/                       # Preserved legacy code
    │   ├── auth-api.ts
    │   ├── index.ts
    │   ├── public-api.ts
    │   ├── types.ts
    ...
    │   └── plugins/
    │
    ├── __temp__/                         # Temporary bridge to legacy AuthAPI
...
## 12. Migration Guide (Legacy → New)

### 12.1 Breaking Changes

| Legacy (v0.x) | New (v1.x) | Notes |
|---------------|-----------|-------|
| `@asgardeo/auth-spa` dependency | `@asgardeo/browser` dependency | Transitive; no direct import needed |
| `app.use(asgardeoPlugin, opts)` | `app.use(AsgardeoPlugin, config)` | Plugin name is PascalCase; config shape changed |
| `state.isSignedIn` (reactive property) | `isSignedIn` (Ref from `useAsgardeo()`) | Access via `.value` in script, direct in template |
| `signIn(config, authCode, sessionState)` | `signIn(options?)` | Simplified; SDK handles code exchange internally |
| `getUser()` → `BasicUserInfo` | `user` (reactive Ref) + `getUserProfile()` | User is reactive; profile includes SCIM2 data |
| `httpRequest()` / `httpRequestAll()` | `http.request()` / `http.requestAll()` | Grouped under `http` namespace |
| `on(Hooks.*, callback)` | Event system TBD | Hook-based events are being redesigned |
| No UI components | Full component library | `<SignIn>`, `<SignedIn>`, `<UserProfile>`, etc. |

### 12.2 Migration Steps

1. Update `package.json`: replace `@asgardeo/auth-spa` with `@asgardeo/vue@^1.0.0`
2. Update plugin registration: `asgardeoPlugin` → `AsgardeoPlugin`; update config shape
3. Update composable usage: destructure reactive refs from `useAsgardeo()`
4. Replace direct API calls with reactive state where available
5. Add `<AsgardeoProvider>` wrapper if not using plugin auto-wrapping
6. Adopt UI components to replace custom sign-in/sign-out UI
...
### 13.4 Optional Peer Dependencies
...
|------|--------|------------|
| **Browser SDK's `AuthAPI` is legacy** | Vue client depends on the same legacy bridge (`__temp__/api`) as React | Accept this as temporary. Structure the Vue client identically to React so that when `@asgardeo/browser` is fully refactored, the migration is mechanical. |
...
| **Breaking changes from legacy Vue SDK** | Existing users on `@asgardeo/vue@0.x` will need migration | Provide clear migration guide. Consider publishing as `@asgardeo/vue@1.0.0` (major version bump). Existing legacy code preserved in `__legacy__/`. |
